...
function parseSheet(sheet) {
    ...
    const commands = {
        ...
        '#TRACK': args => tracks[currentTrack = args[0]] = [],
        '#LOOP': args => loops.push({count: parseInt(args[0]), content: []}),
        '#ENDLOOP': () => {
            const loop = loops.pop();

            (loops.length > 0 ? loops[loops.length - 1].content : tracks[currentTrack]).push(
                ...Array(loop.count).fill(loop.content).flat()
            );
        },
        '#DEFAULT': args => (loops.length > 0 ? loops[loops.length - 1].content : tracks[currentTrack])
            .push(...args.map(cell => {
    ...
    lines.forEach(line => {
        ...
        if (!line.startsWith('#')) line = '#DEFAULT ' + line;
